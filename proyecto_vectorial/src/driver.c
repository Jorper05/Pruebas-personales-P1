...
static float *read_input(const char *path, int *out_n) {
    ...
    if (n == 0) {
        fprintf(stderr, "Error controlado: N = 0. El arreglo debe contener al menos un elemento.\n");
        fclose(f);
        exit(EXIT_FAILURE);
    }
    if (n < 0) {
...
int main(int argc, char **argv) {
    ...
    if (((uintptr_t)in % VEC_ALIGN) != 0 || ((uintptr_t)out % VEC_ALIGN) != 0) {
        fprintf(stderr, "Error interno: memoria no alineada a %d bytes.\n", VEC_ALIGN);
        free(in);
        free(out);
        return EXIT_FAILURE;
    }
    ...
    double stats_total_ms = 0.0;
    double norm_total_ms = 0.0;
    struct timespec t0, t1;

    /* La suma se reporta, pero no forma parte del benchmark principal. */
    sum = sum_array(in, n);

    /* Medir exclusivamente el kernel NASM de estadisticos. */
    for (int r = 0; r < reps; r++) {
        clock_gettime(CLOCK_MONOTONIC, &t0);
        compute_stats(in, n, &mean, &var, &min, &max);
        clock_gettime(CLOCK_MONOTONIC, &t1);
        stats_total_ms += elapsed_ms(t0, t1);
    }

    if (var < 0.0f && fabsf(var) <= 1e-6f)
        var = 0.0f;
    if (var < 0.0f) {
        fprintf(stderr, "Error: la varianza calculada es negativa (%g).\n", (double)var);
        free(in);
        free(out);
        return EXIT_FAILURE;
    }
    ...
    /* Medir exclusivamente el kernel NASM de normalizacion. */
    for (int r = 0; r < reps; r++) {
        clock_gettime(CLOCK_MONOTONIC, &t0);
        normalize_array(in, out, n, mean, stddev);
        clock_gettime(CLOCK_MONOTONIC, &t1);
        norm_total_ms += elapsed_ms(t0, t1);
    }

    /* Promedio de las dos llamadas NASM medidas. */
    double stats_avg_ms = stats_total_ms / reps;
    double norm_avg_ms = norm_total_ms / reps;
    double kernel_avg_ms = stats_avg_ms + norm_avg_ms;
    ...
    printf("Minimo   = %.6f\n", min);
    printf("Maximo   = %.6f\n", max);
    printf("Tiempo promedio compute_stats (%d rep.): %.6f ms\n", reps, stats_avg_ms);
    printf("Tiempo promedio normalize_array (%d rep.): %.6f ms\n", reps, norm_avg_ms);
    printf("Tiempo promedio total de kernels: %.6f ms\n", kernel_avg_ms);

    write_output(output_path, out, n);

    char summary_path[1024];
    snprintf(summary_path, sizeof(summary_path), "%s.stats.txt", output_path);
    write_stats_summary(summary_path, n, sum, mean, var, stddev, min, max, kernel_avg_ms);
